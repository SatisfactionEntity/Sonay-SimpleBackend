...
<div class="row">
    <div class="col-sm-4">
        <div class="box">
            <h2 class="blue">Eventteam</h2>
            <div class="inner">
                <p>Het <?= CMS::$Config['cms']['hotelname'] ?> eventteam zorgt voor de leukste sfeer in Yabbis, de mooiste bouwwerken en de gekste dingen
                    binnen het hotel! De leden zullen jou vermaken met de leukste evenementen in Yabbis!
                    vormen.    <?= CMS::$Config['cms']['hotelname'] ?> eventteam zoekt nog leden solliciteer snel voor 1 van deze functies.</p>
    ...
    <div class="col-sm-8">
        <?php
        $Data = '';
        $Players = CMS::$MySql->query('SELECT username,look,last_online,motto,online,rank FROM users WHERE rank in (14,12) ORDER BY rank DESC');
        $Ranks = CMS::$MySql->query('SELECT id,rank_name,color FROM permissions WHERE id in (14,12) ORDER BY id DESC');
        if (is_array($Ranks))
        {
